...
export default function ProductManagerScreen() {
  ...
  const pick = async () => {
    const ok = await requestGalleryPermission(); // Kiểm tra quyền truy cập ảnh
    if (!ok) {
    ...
    try {
      launchImageLibrary(options, response => {
        ...
        if (response.errorCode) {
          ...
          showErrorToast(`Lỗi chọn ảnh: ${response.errorMessage}`);
          return;
        }
        if (response.assets && response.assets.length > 0) {
          setPicker(response.assets[0]);
        } else {
          showErrorToast('Không chọn được ảnh');
        }
      });
    } catch (error) {
      console.error('Error in Image Picker: ', error);
      showErrorToast('Lỗi khi mở thư viện ảnh');
    }
  };
